...
orderRouter.get("/debug/auth-test", isAuthenticated, async (req, res, next) => {
  try {
    res.json({ 
      success: true, 
...
orderRouter.get("/debug/clerk-test", async (req, res, next) => {
  try {
    let clerkAuth = null;
    let clerkError = null;
    ...
    res.json({ 
      ...
      reqUserId: (req as any).userId,
      clerkAuth,
      clerkError: clerkError ? (clerkError as any).message : null,
      nodeEnv: process.env.NODE_ENV,
      clerkEnabled: Boolean(process.env.CLERK_SECRET_KEY && process.env.NODE_ENV === "production"),
      clerkSecretExists: !!process.env.CLERK_SECRET_KEY,
      clerkPublishableExists: !!process.env.CLERK_PUBLISHABLE_KEY,
...
orderRouter.post("/debug/create-test-order", async (req, res, next) => {
  try {
    ...
    // Create a test order with a placeholder userId
    const testUserId = "debug_test_user_" + Date.now();
    
    // Validate the request data using Zod schema
    let validatedData: any;
    try {
      const { validateCreateOrder } = await import("../domain/validation/order-validation");
    ...
    const transformedOrderItems = validatedData.orderItems.map((item: any) => ({
    ...
    console.error("Error creating debug order:", error);
    next(error);
  }
});
